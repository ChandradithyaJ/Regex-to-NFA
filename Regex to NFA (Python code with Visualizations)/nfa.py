...
class NFA():
	...
	def build_alphabet(self):
		for ch in range(0, len(self.RE)):
			if self.RE[ch] != '+' and self.RE[ch] != '*' and self.RE[ch] != '(' and self.RE[ch] != ')' and self.RE[
				ch] != ' ':
		...
		while i < len(self.RE):
			if self.RE[i] in self.characters and i < len(self.RE):
				if self.RE[i + 1] != '*':
					...
					self.transition_function[(self.current_state, self.RE[i])].add(self.current_state)
					i = i + 1
	...
	def build_NFA(self):
		...
		while i < len(self.RE):
			if self.RE[i] in self.characters and i < len(self.RE):
				...
				if ((i == len(self.RE) - 1 and self.RE[i] == '*') or (
						self.RE[i] == '*' and i < len(self.RE) - 1 and self.RE[i + 1] not in self.characters)):
